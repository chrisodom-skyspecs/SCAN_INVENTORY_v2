/**
 * geo-bin.ts — Geographic grid-cell binning for INVENTORY map clustering
 *
 * Provides a pure utility function `binCaseLocations` that accepts an array of
 * case location coordinates and a Mapbox-style zoom level, and returns an array
 * of bin objects — each representing a grid cell with its center coordinates
 * and a count of how many cases fall within that cell.
 *
 ...
 *
 * Zoom-level coverage examples (approximate square extent per cell):
 *   z  0 →  360° × 170° (1 cell, whole world)
 *   z  3 →   45° × ~37° (512 cells globally)
 *   z  5 →  ~11° × ~9°  (32 768 cells globally)
 *   z  8 →  ~1.4° × ~1° (1 048 576 cells globally)
 *   z 10 → ~0.35° × ~0.3° (~16M cells globally)
 *   z 12 → ~0.09° × ~0.07° (~268M cells globally)
 *
 * A secondary `binByGrid` helper is also exported for simple lat/lng degree
 * rounding (easier to reason about in unit tests and non-Mercator contexts).
 *
 * @module geo-bin
 */
 ...
 *
 * @example
 * // Bin a list of case pins at zoom 8 (≈ 1.4° cell size)
 ...
 *
 * This produces a uniform rectilinear grid that is simpler than tile binning
 * but does not account for the Mercator projection's latitude distortion.
 *
 * @example
 * gridCellSize(0)  // → { lng: 360, lat: 180 }
 * gridCellSize(5)  // → { lng: ~11.25, lat: ~5.625 }
 * gridCellSize(10) // → { lng: ~0.35,  lat: ~0.176 }
 *
 ...
 *   • Unit tests where exact tile math is not needed.
 *   • Non-Mercator map projections.
 *   • Analytics pipelines that require lat/lng-uniform cells.
 *
 * Coordinates that are missing, NaN, or ±Infinity are silently skipped.
 *
 * @example
 * // Two Seattle pins ~0.1° apart, one LA pin — at zoom 5 (cell ≈ 11.25°)
 ...
 *   5,
 * );
 * // → 2 bins (Seattle and LA are in different ~11° cells)
 *
